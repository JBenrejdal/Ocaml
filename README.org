...
En plus de créer votre dépôt git vous devez créer un ticket de déclaration du
binôme *TODO URL POUR CREATION DE TICKET*. Donnez vos nom, vos groupes de TP, le lien vers votre dépôt et le lien vers
votre fichier =README.md= ou =README.org=. Vous pouvez suivre l'exemple donné
dans le ticket #1 *TODO URL DES TICKETS*.
